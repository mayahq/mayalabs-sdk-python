...
def show_post_instruct_options(recipe, session_id):
    ...
    terminal_menu = TerminalMenu(
        menu_entries=menu_entries, menu_cursor_style=menu_cursor_style
    )
    while True:
        ...
        if choice == "1":
            ...
            new_command = input(
                Style.BRIGHT
                + Fore.BLUE
                + "How do you want to modify this program? "
                + Style.RESET_ALL
            )
            ...
            print(
                Style.BRIGHT + Fore.CYAN + "Saved to output.nl file." + Style.RESET_ALL
            )
            exit()
        else:
            print(
...
def print_user_command(command):
    ...
    command_lines = [
        command[i : i + max_width - 8] for i in range(0, len(command), max_width - 8)
    ]
